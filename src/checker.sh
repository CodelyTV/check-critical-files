...
checker::check() {
  ...
  local -r critical_modified_files=$(echo "$patterns" | coll::map_2 str::contains "$modified_files" | sed ':a;N;$!ba;s/\n/<br>/g')

  if [ -n "$critical_modified_files" ]; then
    local -r final_message="$message<br>Modified files:<br>$critical_modified_files"
